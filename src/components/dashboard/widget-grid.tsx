  ...
  return (
    <div className="space-y-4">
            ...
            <>
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-[#273B3A] text-[#E6D4C7] rounded-lg text-xs font-medium hover:bg-[#344948] transition-all"
                >
                ...
                {showAddMenu && addableWidgets.length > 0 && (
                  <>
                    ...
                    />
                    <div className="absolute left-0 top-full mt-1 w-56 bg-[#F0E6E0] border border-[#C9BAB0] rounded-lg shadow-xl z-20 py-1">
                      {addableWidgets.map((w) => (
                        <button
                          key={w.type}
                          onClick={() => handleAddWidget(w.type)}
                          className="w-full text-left px-4 py-2.5 text-xs text-[#4A5654] hover:text-[#1A2726] hover:bg-[#D8CAC0] transition-colors"
                        >
                          {w.title}
                        </button>
                      ))}
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 border border-[#C9BAB0] rounded-lg text-xs text-[#4A5654] hover:text-[#1A2726] hover:bg-[#D8CAC0] transition-all"
              >
        ...
        <button
          ...
          className={cn(
            ...
            editMode
              ...
              : "border border-[#C9BAB0] text-[#4A5654] hover:text-[#1A2726] hover:bg-[#D8CAC0]"
          )}
        >
            ...
            {editMode && (
              <div className="absolute inset-0 z-10 pointer-events-none">
                ...
                <div className="absolute top-2 left-2 pointer-events-auto">
                  <button className="p-1.5 rounded bg-[#D8CAC0] border border-[#C9BAB0] text-[#4A5654] hover:text-[#1A2726] cursor-grab active:cursor-grabbing">
                ...
                <div className="absolute top-2 right-2 flex items-center gap-1 pointer-events-auto">
                  ...
                  <select
                    ...
                    className="px-2 py-1.5 text-[10px] bg-[#D8CAC0] border border-[#C9BAB0] rounded text-[#4A5654] focus:outline-none cursor-pointer"
                  >
                ...
                {/* Edit Mode Border */}
                <div className="absolute inset-0 border-2 border-dashed border-[#273B3A]/30 rounded-xl" />
              </div>
            )}
      ...
      {editMode && hiddenWidgets.length > 0 && (
        <div className="mt-6 p-4 bg-[#E6D4C7] border border-[#C9BAB0] rounded-xl">
          <p className="text-xs font-medium text-[#4A5654] mb-3 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {hiddenWidgets.map((widget) => (
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-[#F0E6E0] border border-[#C9BAB0] rounded-lg text-xs text-[#4A5654] hover:text-[#1A2726] hover:border-[#273B3A]/30 transition-all"
              >
