    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {/* Header */}
          <div className="hidden sm:grid grid-cols-[1fr_2fr_70px_90px_70px_80px_40px] gap-2 text-[10px] text-[#888888] font-semibold uppercase tracking-widest px-1 pb-1">
            <span>Product</span>
          ...
          {lines.map((line) => {
              ...
              return (
                <div key={line.key} className="flex items-center gap-2">
                  <div className="flex items-center gap-2 flex-1">
                    <Type className="w-3.5 h-3.5 text-[#CDB49E] flex-shrink-0" />
                    <input
                      type="text"
            ...
            if (line.type === "note") {
              return (
                <div key={line.key} className="flex items-center gap-2">
                  <div className="flex items-center gap-2 flex-1">
                    <StickyNote className="w-3.5 h-3.5 text-[#888888] flex-shrink-0" />
                    <input
                      type="text"
                      value={line.description}
                      onChange={(e) => updateLine(line.key, "description", e.target.value)}
                      className={`${lineInputClass} flex-1 italic text-[#888888]`}
            ...
            return (
              ...
              >
                ...
                <div className="text-sm text-right font-medium text-[#CDB49E] px-1" title={`Tax: ${formatCurrency(lineTax, currency)}`}>
        ...
        <div className="mt-3 flex items-center gap-4">
          <button
            ...
            className="flex items-center gap-2 text-sm text-[#CDB49E] hover:text-[#d4c0ad] transition-colors font-medium"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm text-[#888888] hover:text-[#f5f0eb] transition-colors font-medium"
          >
      ...
      <div className="flex justify-end">
        <div className="w-72 space-y-2.5 text-sm bg-[#111111] border border-[#2a2a2a] rounded-xl p-5">
          <div className="flex justify-between text-[#888888]">
            <span>Subtotal</span>
            <span className="text-[#f5f0eb] font-medium">{formatCurrency(subtotal, currency)}</span>
          </div>
          <div className="flex justify-between text-[#888888]">
            <span>Tax (HST 13%)</span>
            <span className="text-[#f5f0eb] font-medium">{formatCurrency(totalTax, currency)}</span>
          </div>
          <div className="flex justify-between font-semibold text-base border-t border-[#2a2a2a] pt-3 mt-3">
            <span className="text-[#f5f0eb]">Total ({currency})</span>
            <span className="text-[#CDB49E]">{formatCurrency(total, currency)}</span>
          </div>
        </div>
      </div>
