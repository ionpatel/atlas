...
function AnimatedStatCard({
  ...
  return (
    <div
      ref={ref}
      className="p-3 rounded-xl bg-[#0A0A0A] border border-[#262626] text-left"
      style={{
        animation: visible ? `fadeInScale 0.6s cubic-bezier(.16,1,.3,1) ${delay}ms both` : "none",
      }}
    >
      <div className="flex items-center justify-between mb-1.5">
        <p className="text-[9px] text-[#FAFAFA] uppercase tracking-wider font-medium">
          {label}
        </p>
        <svg
          width={sparkW}
...
function AnimatedBarChart({ visible, delay }: { visible: boolean; delay: number }) {
  ...
  return (
    <div className="h-24 rounded-xl bg-[#0A0A0A] border border-[#262626] flex items-end justify-center gap-1 sm:gap-1.5 px-3 pb-3 pt-2 overflow-hidden">
      {bars.map((h, i) => (
        <div
          key={i}
...
function AnimatedDonutChart({ visible, delay }: { visible: boolean; delay: number }) {
  const segments = [
    ...
    { value: 25, color: '#FAFAFA', label: "Sent" },
    { value: 15, color: "#f87171", label: "Overdue" },
    { value: 15, color: '#FAFAFA', label: "Draft" },
  ];
  const size = 80;
  const thickness = 8;
    ...
    <div className="flex items-center gap-3">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-xs font-bold text-[#FAFAFA]">100</span>
          <span className="text-[7px] text-[#FAFAFA] uppercase">Total</span>
        </div>
      </div>
      <div className="space-y-1">
        {segments.map((seg, i) => (
          ...
          >
            ...
            />
            <span className="text-[8px] text-[#FAFAFA]">{seg.label}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-1.5">
      {items.map((item, i) => (
        <div
          key={i}
          className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-[#0A0A0A]"
          style={{
            opacity: visible ? 1 : 0,
        ...
        >
          ...
          />
          <span className="text-[8px] sm:text-[9px] text-[#FAFAFA] truncate flex-1">
            {item.text}
          </span>
          <span className="text-[7px] text-[#FAFAFA] flex-shrink-0">{item.time}</span>
        </div>
      ))}
    </div>
  );
...
function FloatingInvoiceCard({ visible, delay }: { visible: boolean; delay: number }) {
  return (
    ...
    >
      <div className="bg-[#0A0A0A] border border-[#262626] rounded-xl p-3 shadow-2xl shadow-black/60">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[8px] font-mono text-[#FAFAFA]">INV-1042</span>
          <span className="text-[7px] px-1.5 py-0.5 rounded-full bg-[#34d399]/10 text-[#34d399] font-medium border border-[#34d399]/20">
            Paid
          </span>
        </div>
        <p className="text-xs font-semibold text-[#FAFAFA]">$2,450.00</p>
        <p className="text-[8px] text-[#FAFAFA] mt-1">Maple Leaf Co.</p>
        <div className="mt-2 h-px bg-[#0A0A0A]" />
        <p className="text-[7px] text-[#FAFAFA] mt-1.5">Due: Feb 15, 2026</p>
      </div>
    </div>
  );
}
    ...
    >
      <div className="bg-[#0A0A0A] border border-[#262626] rounded-xl p-3 shadow-2xl shadow-black/60">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-[8px] font-semibold text-[#FAFAFA]">Payment Received</span>
        </div>
        <p className="text-[8px] text-[#FAFAFA] leading-relaxed">
    ...
    >
      <div className="bg-[#0A0A0A] border border-[#262626] rounded-xl p-3 shadow-2xl shadow-black/60">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[8px] font-semibold text-[#FAFAFA] truncate">Widget Pro X</span>
        </div>
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[7px] text-[#FAFAFA]">Stock Level</span>
        ...
        <p className="text-[7px] text-[#FAFAFA] mt-1.5">12 / 50 units</p>
      </div>
    </div>
  );
}
...
function FloatingAIChatBubble({ visible, delay }: { visible: boolean; delay: number }) {
  return (
    ...
    >
      <div
        className="bg-[#0A0A0A] border border-[#262626]/20 rounded-xl p-3 shadow-2xl shadow-black/60"
        style={{
      ...
      >
        <div className="flex items-center gap-1.5 mb-2">
          <div className="w-4 h-4 rounded-lg bg-[rgba(156,74,41,0.15)] flex items-center justify-center">
            <Sparkles size={8} className="text-[#FAFAFA]" />
          </div>
          <span className="text-[8px] font-semibold text-[#FAFAFA]">Atlas AI</span>
        </div>
        <p className="text-[8px] text-[#FAFAFA] leading-relaxed">
...
function AnimatedDashboardMockup() {
  ...
  return (
    <div ref={ref} className="mt-20 mx-auto max-w-3xl relative">
      ...
      {/* Main dashboard card */}
      <div
        className="rounded-2xl border border-[#262626] bg-[#0A0A0A] overflow-hidden shadow-2xl shadow-black/40"
        style={{
          animation: visible ? "fadeInScale 0.8s cubic-bezier(.16,1,.3,1) both" : "none",
        }}
      >
        {/* Browser chrome */}
        <div className="flex items-center gap-2 px-5 py-3 border-b border-[#262626] bg-[#0A0A0A]">
          ...
          <div className="flex-1 text-center">
            <div className="inline-block px-4 py-1 rounded-md bg-[#0A0A0A] text-[10px] text-[#FAFAFA] font-mono">
        ...
        <div className="p-4 sm:p-6 min-h-[320px] sm:min-h-[380px]">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4">
            ...
            <div className="rounded-xl bg-[#0A0A0A] border border-[#262626] p-2.5 flex items-center justify-center">
              <AnimatedDonutChart visible={visible} delay={1000} />
            </div>
          </div>

          {/* Activity feed */}
          <div className="rounded-xl bg-[#0A0A0A] border border-[#262626] p-2.5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[8px] sm:text-[9px] font-semibold text-[#FAFAFA] uppercase tracking-wider">
                Recent Activity
              </span>
              <span className="text-[7px] text-[#FAFAFA]">Live</span>
            </div>
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-[#0A0A0A]/80 backdrop-blur-2xl shadow-[0_1px_0_0_rgba(255,255,255,0.04)]"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto max-w-6xl px-6 flex items-center justify-between h-[72px]">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-9 h-9 rounded-lg bg-[#161616] flex items-center justify-center font-bold text-sm text-[#0A0A0A] transition-transform duration-300 group-hover:scale-105">
            A
          </div>
      ...
      {/* Mobile drawer */}
      {menuOpen && (
        <div className="md:hidden bg-[#0A0A0A]/98 backdrop-blur-2xl border-t border-[#262626] px-6 pb-8 pt-6 space-y-5">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
          ...
          <div className="flex items-center gap-5 pt-3">
            <Link href="/login" className="text-sm text-[#FAFAFA]">
              Log in
            </Link>
            <Link
              href="/signup"
              className="text-sm font-medium px-6 py-2.5 rounded-full bg-[#161616] text-[#0A0A0A]"
            >
              Start Free Trial
            </Link>
          </div>
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-[72px] overflow-hidden bg-[#0A0A0A]">
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6 text-center py-20">
        <Reveal>
          <div className="inline-flex items-center gap-2.5 px-5 py-2 rounded-full bg-[rgba(156,74,41,0.15)]/60 text-[#FAFAFA] text-sm font-medium mb-10 border border-[#262626]/15 backdrop-blur-sm">
            <Sparkles size={14} />
        ...
        <Reveal delay={120}>
          <h1 className="text-4xl sm:text-5xl lg:text-[3.75rem] font-bold tracking-tight text-[#FAFAFA] leading-[1.08]">
            Run your business
            <br />
            <span className="text-[#FAFAFA]">smarter, not harder</span>
          </h1>
        </Reveal>

        <Reveal delay={240}>
          <p className="mt-7 text-lg sm:text-xl text-[#FAFAFA] max-w-2xl mx-auto leading-relaxed">
            ...
            <span className="text-[#FAFAFA] font-semibold">$9/user/month</span>.
          </p>
        </Reveal>
        ...
        <Reveal delay={360}>
          <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link
              href="/dashboard"
              className="inline-flex items-center gap-2.5 px-8 py-4 rounded-full bg-transparent text-[#FAFAFA] font-semibold text-base hover:bg-[#0A0A0A] transition-all duration-300 border border-[#262626] hover:border-[rgba(156,74,41,0.15)]"
            >
              See Live Demo
            </Link>
          </div>
        ...
        <Reveal delay={440}>
          <p className="mt-6 text-xs text-[#FAFAFA]">
...
function SocialProof() {
  ...
  return (
    <Reveal className="py-16 border-y border-[#262626] bg-[#0A0A0A]">
      <div className="mx-auto max-w-6xl px-6 text-center">
        <p className="text-[11px] text-[#FAFAFA] mb-10 tracking-[0.25em] uppercase font-medium">
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-14 gap-y-5">
          {logos.map((name) => (
            <span
              key={name}
              className="text-[#0A0A0A] font-semibold text-base tracking-tight select-none hover:text-[#FAFAFA] transition-colors duration-500"
            >
              {name}
            </span>
          ))}
        ...
        <Reveal>
          <div className="text-center max-w-2xl mx-auto mb-20">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-[#FAFAFA]">
              ...
              <br />
              We charge{" "}
              <span className="text-[#FAFAFA]">$9</span>.
            </h2>
            <p className="mt-5 text-[#FAFAFA] text-lg">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {problems.map((p, i) => (
            <Reveal key={p.title} delay={i * 100}>
              <div className="group p-7 rounded-2xl bg-[#0A0A0A] border border-[#262626] hover:border-[rgba(156,74,41,0.15)] hover:bg-[#1e1e1e] transition-all duration-500">
                <span className="text-3xl">{p.emoji}</span>
                <h3 className="text-[#FAFAFA] font-semibold text-base mt-5 mb-2.5">
                  {p.title}
                </h3>
  ...
  return (
    <section className="py-28 sm:py-36 bg-[#0e0e0e]" id="features">
      <div className="mx-auto max-w-6xl px-6">
        <Reveal>
          <div className="text-center max-w-2xl mx-auto mb-20">
            <p className="text-sm font-medium text-[#FAFAFA] mb-4 tracking-wide uppercase">
              Everything you need
            </p>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-[#FAFAFA]">
          ...
          {features.map((f, i) => {
            ...
            return (
              <Reveal key={f.title} delay={i * 80}>
                <div className="group relative p-8 rounded-2xl bg-[#0A0A0A] border border-[#262626] hover:border-[#262626]/20 transition-all duration-500 h-full">
                  {f.badge && (
                    <span className="absolute top-6 right-6 text-[10px] font-semibold uppercase tracking-widest px-3 py-1.5 rounded-full bg-[rgba(156,74,41,0.15)] text-[#FAFAFA] border border-[#262626]/15">
                      Only in Atlas
                    </span>
                  )}
                  <div className="w-12 h-12 rounded-xl bg-[rgba(156,74,41,0.15)] flex items-center justify-center mb-6 group-hover:bg-[#161616]/15 transition-colors duration-500">
                    <Icon size={22} className="text-[#FAFAFA]" />
                  </div>
                  <h3 className="text-[#FAFAFA] font-semibold text-base mb-2.5">
                    {f.title}
                  </h3>
                  <p className="text-[#FAFAFA] text-sm leading-relaxed">
                    {f.desc}
...
function Comparison() {
  ...
  const renderCell = (v: string | boolean) => {
    ...
    return <span className="text-sm text-[#FAFAFA] font-medium">{String(v)}</span>;
  };

  return (
    <section className="py-28 sm:py-36 bg-[#0A0A0A]" id="compare">
      <div className="mx-auto max-w-4xl px-6">
        ...
        <Reveal delay={100}>
          <div className="rounded-2xl border border-[#262626] overflow-hidden bg-[#0A0A0A]">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-[#262626]">
                    <th className="p-5 text-sm font-medium text-[#FAFAFA]">
                      Feature
                    </th>
                    <th className="p-5 text-sm font-semibold text-[#FAFAFA] text-center">
                      <div className="flex flex-col items-center gap-1.5">
                        <div className="w-7 h-7 rounded-md bg-[#161616] flex items-center justify-center text-xs font-bold text-[#0A0A0A]">
                          A
                        </div>
                        Atlas
                      </div>
                    </th>
                    <th className="p-5 text-sm font-medium text-[#FAFAFA] text-center">
                      Odoo
                    </th>
                    <th className="p-5 text-sm font-medium text-[#FAFAFA] text-center">
                      SAP
                  ...
                  {rows.map((row, i) => (
                    <tr
                      ...
                      className={`border-b border-[#d4c0b1] ${
                        i % 2 === 0 ? "bg-[#0A0A0A]" : "bg-[#dcc8b9]"
                      }`}
                    >
                      <td className="p-5 text-sm text-[#FAFAFA] font-medium">
                        {row.feature}
                      </td>
                      <td className="p-5 text-center font-medium text-[#FAFAFA]">
        ...
        <div className="grid md:grid-cols-3 gap-5 max-w-4xl mx-auto">
          {plans.map((plan, i) => (
            <Reveal key={plan.name} delay={i * 120}>
              ...
              >
                {plan.popular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-5 py-1.5 bg-[#161616] text-[#0A0A0A] text-xs font-bold rounded-full tracking-wide uppercase">
                    Most Popular
                  </div>
                )}
                ...
                <p className="text-sm text-[#FAFAFA] mt-1">{plan.desc}</p>
                <div className="mt-7 mb-7">
                  <span className="text-5xl font-bold text-[#FAFAFA]">
                    ${plan.price}
                  </span>
                  <span className="text-[#FAFAFA] text-sm ml-1.5">
                    /user/mo
                  </span>
                </div>
                <Link
                ...
                <ul className="mt-8 space-y-3.5">
                  {plan.features.map((f) => (
                    <li
                      key={f}
                      className="flex items-start gap-3 text-sm text-[#FAFAFA]"
                    >
                      <Check
                        size={16}
                        className={`shrink-0 mt-0.5 ${
                          plan.popular ? "text-[#FAFAFA]" : "text-[#FAFAFA]"
                        }`}
                      />
                      {f}
                    </li>
...
function Testimonials() {
  return (
    <section className="py-28 sm:py-36 bg-[#0A0A0A]" id="testimonials">
      <div className="mx-auto max-w-6xl px-6">
        <Reveal>
          <div className="text-center max-w-2xl mx-auto mb-20">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-[#FAFAFA]">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {testimonials.map((t, i) => (
            <Reveal key={t.name} delay={i * 120}>
              <div className="group p-8 rounded-2xl bg-[#0A0A0A] border border-[#262626] hover:border-[rgba(156,74,41,0.15)] transition-all duration-500 h-full flex flex-col">
                ...
                <div className="mt-7 pt-6 border-t border-[#262626]">
                  <p className="text-sm font-semibold text-[#FAFAFA]">
                    {t.name}
                  </p>
                  <p className="text-xs text-[#FAFAFA] mt-1 flex items-center gap-1.5">
...
function FinalCTA() {
  ...
  return (
    <section className="relative py-28 sm:py-36 bg-[#0e0e0e] overflow-hidden">
      {/* Ambient glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-[#161616]/[0.03] rounded-full blur-[100px] pointer-events-none" />
      ...
      <Reveal>
        <div className="relative z-10 mx-auto max-w-3xl px-6 text-center">
          ...
          <p className="mt-5 text-[#FAFAFA] text-lg max-w-xl mx-auto">
          ...
          >
            <input
              ...
              required
              className="flex-1 px-6 py-3.5 rounded-full bg-[#0A0A0A] border border-[#262626] text-[#FAFAFA] placeholder:text-[#FAFAFA] text-sm focus:outline-none focus:ring-2 focus:ring-[#CDB49E]/20 focus:border-[#262626]/40 transition-all duration-300"
            />
            <button
              type="submit"
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10">
          ...
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="flex items-center gap-2.5 mb-5">
              <div className="w-8 h-8 rounded-lg bg-[#161616] flex items-center justify-center font-bold text-xs text-[#0A0A0A]">
                A
              </div>
            ...
            <p className="mt-3 text-sm text-[#FAFAFA]">Built in Canada 🇨🇦</p>
          </div>
          {/* Link columns */}
          {columns.map((col) => (
            <div key={col.title}>
              <h4 className="text-xs font-semibold text-[#FAFAFA] uppercase tracking-[0.2em] mb-5">
                {col.title}
              </h4>
          ...
          <div className="flex items-center gap-6">
            {["Twitter", "LinkedIn", "GitHub"].map((s) => (
              <a
                key={s}
                href="#"
                className="text-xs text-[#444444] hover:text-[#FAFAFA] transition-colors duration-300"
              >
                {s}
              </a>
            ))}
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#FAFAFA] overflow-x-hidden antialiased">
      <Nav />
      <Hero />
      <SocialProof />
      <ProblemSolution />
