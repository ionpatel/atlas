...
function TypeBadge({ type }: { type: string }) {
  const styles: Record<string, string> = {
    ...
    both: "bg-[rgba(156,74,41,0.15)] text-[#111827] border-[#E5E7EB]/20",
  };
...
function ContactDetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <div
        className={`relative w-full max-w-lg bg-[#F8F9FA] border-l-4 ${borderColor} shadow-float animate-in slide-in-from-right duration-300 h-full overflow-y-auto`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 bg-[#F8F9FA] border-b border-[#E5E7EB] px-6 py-5 z-10">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg font-semibold text-[#111827]">{contact.name}</h2>
                <div className="flex items-center gap-2 mt-1">
                  {contact.company && (
                    <span className="text-sm text-[#111827]">{contact.company}</span>
                  )}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-lg text-[#374151] hover:text-[#111827] hover:bg-[#F8F9FA] transition-all duration-200"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="flex items-center gap-2 mt-4">
            {contact.email && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#374151] hover:text-[#111827] border border-[#E5E7EB] rounded-lg hover:border-[#E5E7EB]/30 hover:bg-[rgba(156,74,41,0.15)]/30 transition-all duration-200"
              >
            ...
            {(["overview", "invoices", "activity"] as const).map((t) => (
              <button
                ...
                className={`px-4 py-2.5 text-sm font-medium rounded-t-lg border-b-2 transition-all duration-200 ${
                  tab === t
                    ? "border-[#E5E7EB] text-[#111827] bg-[#F8F9FA]"
                    : "border-transparent text-[#374151] hover:text-[#111827]"
                }`}
              >
                {t.charAt(0).toUpperCase() + t.slice(1)}
                {t === "invoices" && contactInvoices.length > 0 && (
                  <span className="ml-1.5 px-1.5 py-0.5 text-[10px] bg-[rgba(156,74,41,0.15)] text-[#111827] rounded-full">
        ...
        <div className="px-6 py-6">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 gap-4">
                {[
                  ...
                  (field) =>
                    field.value && (
                      <div key={field.label} className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-lg p-4">
                        <div className="flex items-center gap-2 mb-1">
                          <field.icon className="w-3.5 h-3.5 text-[#111827]" />
                          <span className="text-[10px] font-semibold text-[#111827] uppercase tracking-widest">{field.label}</span>
                        </div>
                        <p className="text-sm text-[#111827] pl-5.5">{field.value}</p>
                      </div>
                    )
                )}
                {contact.notes && (
                  <div className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Notes</span>
                    </div>
                    <p className="text-sm text-[#111827]">{contact.notes}</p>
                  </div>
                )}
                <div className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <Clock className="w-3.5 h-3.5 text-[#111827]" />
                    <span className="text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Added</span>
                  </div>
                  <p className="text-sm text-[#111827]">{formatDate(contact.created_at)}</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              {contactInvoices.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-[#111827]">No invoices for this contact</p>
                </div>
              ) : (
                contactInvoices.map((inv) => {
                  const statusColor: Record<string, string> = {
                    ...
                    draft: "text-[#111827]",
                    cancelled: "text-[#111827]",
                  };
                  return (
                    <div
                      key={inv.id}
                      className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-lg p-4 hover:border-[#E5E7EB]/25 transition-all duration-200"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="text-sm font-mono font-medium text-[#111827]">{inv.invoice_number}</span>
                          <p className="text-xs text-[#111827] mt-0.5">{formatDate(inv.issue_date)}</p>
                        </div>
                        <div className="text-right">
                          <span className="text-sm font-semibold text-[#111827]">{formatCurrency(inv.total)}</span>
                          <p className={`text-xs mt-0.5 capitalize ${statusColor[inv.status] || "text-[#111827]"}`}>
                            {inv.status}
                          </p>
                        </div>
                      </div>
          ...
          {tab === "activity" && (
            <div className="text-center py-12">
              <Clock className="w-8 h-8 mx-auto mb-3 text-[#111827]/40" />
              <p className="text-sm text-[#111827]">No activity yet</p>
              <p className="text-xs text-[#111827] mt-1">Activity will appear here once you interact with this contact</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="space-y-6 max-w-[1400px]">
          ...
          <div className="relative">
            <div className="flex items-center border border-[#E5E7EB] rounded-lg overflow-hidden">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium text-[#374151] hover:text-[#111827] hover:bg-[#F8F9FA] transition-all duration-200"
              >
      ...
      {viewMode === "grid" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.length === 0 ? (
            <div className="col-span-full text-center py-16 text-[#111827] text-sm">
              <Users className="w-8 h-8 mx-auto mb-3 text-[#111827]/40" />
              No contacts found
            </div>
          ) : (
            filtered.map((c) => (
              <div
                ...
                className={`bg-[#F8F9FA] border border-[#E5E7EB] border-l-4 ${borderColorForType(c.type)} rounded-xl p-6 hover:border-[#E5E7EB]/25 transition-all duration-300 cursor-pointer group`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="w-11 h-11 rounded-full bg-[rgba(156,74,41,0.15)] flex items-center justify-center group-hover:bg-white/20 transition-colors duration-300">
                    ...
                    <div className="hidden group-hover:flex items-center gap-1">
                      {c.email && (
                        <a
                          href={`mailto:${c.email}`}
                          onClick={(e) => e.stopPropagation()}
                          className="p-1.5 rounded-lg text-[#374151] hover:text-[#111827] hover:bg-[rgba(156,74,41,0.15)] transition-all duration-200"
                          title="Email"
                        >
                          <Mail className="w-3 h-3" />
                        </a>
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-[#111827] hover:text-red-400 hover:bg-red-500/10 transition-all duration-200"
                        title="Delete"
                      >
                ...
                <h3 className="font-semibold text-sm text-[#111827]">{c.name}</h3>
                <p className="text-xs text-[#111827] mt-0.5">{c.company || "—"}</p>
                <div className="mt-4 space-y-2 border-t border-[#E5E7EB] pt-4">
                  {c.email && (
                    <div className="flex items-center gap-2.5 text-xs text-[#111827]">
                      <Mail className="w-3 h-3 text-[#111827]/60" />
                      <span className="truncate">{c.email}</span>
                    </div>
                  )}
                  {c.phone && (
                    <div className="flex items-center gap-2.5 text-xs text-[#111827]">
                      <Phone className="w-3 h-3 text-[#111827]/60" />
                      {c.phone}
                    </div>
                  )}
                </div>
                <div className="mt-3 flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <span className="text-xs text-[#111827] flex items-center gap-1">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E5E7EB]">
                <th className="text-left px-6 py-4 text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Name</th>
                <th className="text-left px-6 py-4 text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Company</th>
                <th className="text-left px-6 py-4 text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Email</th>
                <th className="text-left px-6 py-4 text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Phone</th>
                <th className="text-right px-6 py-4 text-[10px] font-semibold text-[#111827] uppercase tracking-widest">Type</th>
                <th className="text-right px-6 py-4 text-[10px] font-semibold text-[#111827] uppercase tracking-widest w-32">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((c, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-[#111827]">{c.company || "—"}</td>
                    <td className="px-6 py-4 text-sm text-[#111827]">{c.email || "—"}</td>
                    <td className="px-6 py-4 text-sm text-[#111827]">{c.phone || "—"}</td>
