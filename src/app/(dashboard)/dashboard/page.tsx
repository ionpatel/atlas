  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[10px] text-[#111827] uppercase tracking-wider">
          Total
        </span>
      </div>
    </div>
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
            <span className="text-[10px] text-[#111827] opacity-0 group-hover:opacity-100 transition-opacity font-mono">
            ...
            <span className="text-[10px] text-[#111827] uppercase tracking-wider whitespace-nowrap">
              {d.label}
            </span>
          </div>
        );
...
function InvoiceStatusIcon({ status }: { status: string }) {
  switch (status) {
    ...
    case "sent":
      return <Send className="w-3.5 h-3.5 text-[#111827]" />;
    case "overdue":
      return <AlertCircle className="w-3.5 h-3.5 text-red-400" />;
    case "draft":
      return <CircleDot className="w-3.5 h-3.5 text-[#111827]" />;
    default:
      return <CircleDot className="w-3.5 h-3.5 text-[#111827]" />;
  }
}
...
export default function DashboardPage() {
  ...
  const invoiceSegments = useMemo(
    () => [
      ...
      { value: stats.sentCount, color: '#111827', label: "Sent" },
      { value: stats.overdueCount, color: "#f87171", label: "Overdue" },
      { value: stats.draftCount, color: '#111827', label: "Draft" },
    ],
    [stats]
  );
  ...
  const favoriteModules = [
    ...
    { label: "Invoices", icon: FileText, href: "/invoices", color: "text-[#111827]", bg: "bg-[rgba(156,74,41,0.15)]" },
  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      ...
      {/* ── Favorites bar ── */}
      <div className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-xl">
        <div className="px-5 py-3 border-b border-[#E5E7EB] flex items-center gap-2">
          <Star className="w-3.5 h-3.5 text-[#111827]" />
          <span className="text-xs font-semibold text-[#111827] uppercase tracking-wider">Favorites</span>
        </div>
        <div className="flex items-center gap-1 px-3 py-2.5 overflow-x-auto">
          {favoriteModules.map((mod) => (
            <Link
              key={mod.label}
              href={mod.href}
              className="flex items-center gap-2 px-3.5 py-2 rounded-lg hover:bg-[#F8F9FA] transition-all duration-200 flex-shrink-0 group"
            >
              <div className={`p-1.5 rounded-md ${mod.bg}`}>
                <mod.icon className={`w-3.5 h-3.5 ${mod.color}`} />
              </div>
              <span className="text-xs font-medium text-[#111827] group-hover:text-[#111827] transition-colors">
                {mod.label}
              </span>
            </Link>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Revenue */}
        <div className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-xl p-5 hover:border-[#E5E7EB]/20 transition-all duration-300 group">
          ...
          <div className="flex items-center justify-between mt-2">
            <p className="text-xs text-[#111827] uppercase tracking-wider">
              Revenue (Paid)
            </p>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-[11px]">
              <span className="text-[#111827]">
                {stats.customerCount}{" "}
                <span className="text-[#111827]">cust</span>
              </span>
              <span className="text-[#111827]">
                {stats.vendorCount}{" "}
                <span className="text-[#111827]">vend</span>
              </span>
            </div>
          </div>
          <p className="text-2xl font-bold text-[#111827] tracking-tight">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Revenue trend chart */}
        <div className="lg:col-span-2 bg-[#F8F9FA] border border-[#E5E7EB] rounded-xl">
          <div className="px-6 py-5 border-b border-[#E5E7EB] flex items-center justify-between">
            ...
            <span className="text-[11px] text-[#111827]">Last 7 months</span>
          </div>
          <div className="p-6">
        ...
        <div className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-xl">
          <div className="px-6 py-5 border-b border-[#E5E7EB] flex items-center justify-between">
            ...
            <Link
              href="/invoices"
              className="text-[11px] text-[#374151] hover:text-[#111827] transition-colors flex items-center gap-1"
            >
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2.5 w-full">
              {invoiceSegments.map((seg) => (
                <div key={seg.label} className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-xs text-[#111827]">{seg.label}</span>
                  <span className="text-xs font-semibold text-[#111827] ml-auto">
                    {seg.value}
                  </span>
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div>
            {recentInvoices.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <FileText className="w-8 h-8 mx-auto mb-3 text-[#111827]/30" />
                <p className="text-sm text-[#111827]">No invoices yet</p>
              </div>
            ) : (
              recentInvoices.map((inv, i) => (
                <div
                  key={inv.id}
                  className={classNames(
                    "px-6 py-4 flex items-center justify-between transition-colors hover:bg-[#F8F9FA]/50 cursor-pointer",
                    i < recentInvoices.length - 1 &&
                      "border-b border-[#E5E7EB]/50"
                  )}
                >
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-9 h-9 rounded-lg bg-[#F8F9FA] flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#111827] mt-0.5 truncate">
                  ...
                  <div className="text-right flex-shrink-0 ml-4">
                    ...
                    <p className="text-[11px] text-[#111827] mt-0.5">
        ...
        <div className="bg-[#F8F9FA] border border-[#E5E7EB] rounded-xl">
          <div className="px-6 py-5 border-b border-[#E5E7EB] flex items-center justify-between">
            ...
            <span className="text-[11px] text-[#111827]">Last 7 days</span>
          </div>
          <div className="p-6">
                ...
                <div key={i} className="flex gap-4 group">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {i < TIMELINE_EVENTS.length - 1 && (
                      <div className="w-px flex-1 bg-[#F8F9FA] my-1" />
                    )}
                  </div>
                  ...
                  <div className={classNames("pb-6 min-w-0 flex-1", i === TIMELINE_EVENTS.length - 1 && "pb-0")}>
                    ...
                    <p className="text-xs text-[#111827] mb-1">{event.description}</p>
                    <p className="text-[11px] text-[#111827]">{event.time}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          {topProducts.length === 0 ? (
            <div className="px-6 py-12 text-center col-span-2">
              <Package className="w-8 h-8 mx-auto mb-3 text-[#111827]/30" />
              <p className="text-sm text-[#111827]">No products yet</p>
            </div>
          ) : (
            topProducts.map((p, i) => (
              <div
                key={p.id}
                className={classNames(
                  "px-6 py-4 flex items-center justify-between transition-colors hover:bg-[#F8F9FA]/50 cursor-pointer border-b border-[#E5E7EB]/50"
                )}
              >
                <div className="flex items-center gap-4 min-w-0">
