...
function PlanCard({
  ...
  return (
    <div
      className={cn(
        "relative bg-[#1a1a1a] border rounded-xl p-6 transition-all",
        plan.popular
          ? "border-[#CDB49E] ring-1 ring-[#CDB49E]/20"
          : "border-[#2a2a2a] hover:border-[#3a3a3a]",
        isCurrent && "bg-[#1f1f1f]"
      )}
    >
      {plan.popular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-[#CDB49E] text-[#111111] text-xs font-semibold rounded-full">
          Most Popular
        </div>
      )}

      <div className="mb-4">
        <h3 className="text-lg font-semibold text-[#f5f0eb]">{plan.name}</h3>
        <p className="text-sm text-[#888888] mt-1">{plan.description}</p>
      </div>

      <div className="mb-6">
        <span className="text-3xl font-bold text-[#f5f0eb]">
          {plan.price === 0 ? "Free" : formatPrice(plan.price)}
        </span>
        {plan.price > 0 && (
          <span className="text-[#888888] text-sm">/{plan.interval}</span>
        )}
      </div>
      ...
      <button
        ...
        className={cn(
          ...
          isCurrent
            ? "bg-[#222222] text-[#888888] cursor-default"
            : plan.popular
            ? "bg-[#CDB49E] text-[#111111] hover:bg-[#d4c0ad]"
            : "bg-[#222222] text-[#f5f0eb] hover:bg-[#2a2a2a]",
...
function PaymentMethodCard({ method }: { method: PaymentMethod }) {
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg">
      ...
      {method.isDefault && (
        <span className="px-2 py-1 bg-[#CDB49E]/10 text-[#CDB49E] text-xs font-medium rounded">
          Default
        </span>
      )}
    </div>
...
function InvoiceRow({ invoice }: { invoice: BillingInvoice }) {
  const statusColors: Record<string, string> = {
    ...
    void: "text-[#555555] bg-[#1a1a1a]",
    uncollectible: "text-red-400 bg-red-500/10",
  };

  return (
    <div className="flex items-center justify-between py-3 border-b border-[#2a2a2a]/50 last:border-0">
      ...
      <div className="flex items-center gap-3">
        ...
        {invoice.invoicePdf && (
          <a
            ...
            className="p-1.5 text-[#888888] hover:text-[#f5f0eb] transition-colors"
          >
...
export default function BillingPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <Loader2 className="w-8 h-8 animate-spin text-[#CDB49E]" />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1200px]">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {stripeConfigured && subscription?.plan !== "free" && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#222222] text-[#f5f0eb] rounded-lg text-sm font-medium hover:bg-[#2a2a2a] transition-all"
          >
            {portalLoading ? (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#CDB49E] to-[#a08c75] flex items-center justify-center">
              <Crown className="w-6 h-6 text-[#111111]" />
            </div>
            <div>
              <p className="text-sm text-[#888888]">Current Plan</p>
              <p className="text-xl font-semibold text-[#f5f0eb] capitalize">
      ...
      {/* Pricing Plans */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-[#f5f0eb] mb-4">Available Plans</h2>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[#f5f0eb]">Payment Methods</h2>
          {stripeConfigured && (
            <button
              onClick={handleManageBilling}
              className="text-sm text-[#CDB49E] hover:text-[#d4c0ad] transition-colors"
            >
              Add Payment Method
            </button>
          )}
        ...
        {paymentMethods.length > 0 ? (
          ...
          <div className="p-8 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl text-center">
            ...
            <p className="text-[#888888] text-sm">No payment methods on file</p>
      ...
      {/* Billing History */}
      <div>
        <h2 className="text-lg font-semibold text-[#f5f0eb] mb-4">Billing History</h2>
        {invoices.length > 0 ? (
          ...
          <div className="p-8 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl text-center">
            ...
            <p className="text-[#888888] text-sm">No invoices yet</p>
            <p className="text-[#555555] text-xs mt-1">
      ...
      <div className="mt-8 p-6 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl">
        <h2 className="text-lg font-semibold text-[#f5f0eb] mb-4">Why Upgrade?</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-lg bg-[#CDB49E]/10">
              <Users className="w-4 h-4 text-[#CDB49E]" />
            </div>
            <div>
              <p className="text-sm font-medium text-[#f5f0eb]">Unlimited Users</p>
          ...
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-lg bg-[#CDB49E]/10">
              <Code className="w-4 h-4 text-[#CDB49E]" />
            </div>
            <div>
              <p className="text-sm font-medium text-[#f5f0eb]">API Access</p>
              <p className="text-xs text-[#888888] mt-1">
          ...
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-lg bg-[#CDB49E]/10">
              <Shield className="w-4 h-4 text-[#CDB49E]" />
            </div>
            <div>
              <p className="text-sm font-medium text-[#f5f0eb]">Priority Support</p>
              <p className="text-xs text-[#888888] mt-1">
